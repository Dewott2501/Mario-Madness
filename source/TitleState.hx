...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		arms = new FlxSprite().loadGraphic(Paths.image('Title_Screen_Assests_MarioArms'));
		arms.antialiasing = ClientPrefs.globalAntialiasing;
		arms.setGraphicSize(Std.int(arms.width * 0.9));
		arms.updateHitbox();
		arms.screenCenter(X);
		add(arms);

		couple = new FlxSprite(0, 400);
		couple.frames = Paths.getSparrowAtlas('Title_Screen_Assests_BFGF');
		couple.animation.addByPrefix('idle', "BF n GF sprites", 12, false);
		couple.antialiasing = ClientPrefs.globalAntialiasing;
		couple.setGraphicSize(Std.int(couple.width * 0.9));
		couple.updateHitbox();
		couple.screenCenter(X);
		add(couple);

		logoBl = new FlxSprite(0, 50).loadGraphic(Paths.image('logoexe'));
		...
		titleText.frames = Paths.getSparrowAtlas('Title_Screen_Assests_Enter');
		titleText.animation.addByPrefix('press', "textoSexoso", 20);
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(arms, {y: 20}, 3, {ease: FlxEase.quadInOut, type: PINGPONG});
		}
	}
}
